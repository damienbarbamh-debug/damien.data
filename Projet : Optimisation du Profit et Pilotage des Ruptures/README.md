Objectif du Projet : 
L'enjeu : Transformer une gestion de stock purement quantitative en un pilotage stratégique par la valeur.


La problématique : Identifier les risques de rupture sur les produits à forte contribution de marge pour garantir la rentabilité de l'entreprise (Spécialisation : Inventory Optimization Analyst ).

Points Clés du Dashboard : 

Analyse de la Valeur : Identification des catégories (Maison, Tech) et des canaux (Web) générant la plus forte marge brute (Expertise : Revenue Analyst ).

Matrice d'Alerte Dynamique : Comparaison en temps réel entre le stock disponible et le seuil d'alerte, classée par importance de marge via la Loi de Pareto.


Calcul du Manque à Gagner : Mesure DAX spécifique isolant le profit "perdu" ou "menacé" par les ruptures de stock actuelles (Rôle : Financial Data Analyst ).


Priorisation Logistique : Extraction automatique d'une "Action List" (Top 5 produits prioritaires) pour sécuriser la croissance (Métier : Logistics Data Analyst ).

Impact Business : 

Diagnostic de Rentabilité : Le dashboard révèle que 26,57% de la marge totale est actuellement menacée par des ruptures de stock (Spécialisation : Risk Modeling Specialist ).


Optimisation des Flux : Priorisation des approvisionnements sur les produits "Stars" (ex: Canapé Velvet, iPhone 15) pour maximiser le ROI (Approche : Optimization Analyst ).


Efficacité Opérationnelle : Optimisation de l'espace en entrepôt en isolant les stocks à faible rotation des produits critiques (Rôle : Operations Analyst ).


Performance Financière : Réduction directe du manque à gagner financier lié aux défauts de disponibilité (Expertise : Business Data Analyst ).

🔗 Ressources Techniques
Schéma SQL (Base de données) : Accéder au DB-Fiddle

Rapport Interactif Power BI : Voir le Dashboard
